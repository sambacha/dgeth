...
describe('GethProvider', () => {
  ...
  it('send ETH transfer', async () => {
    ...
    expect((await provider.getTransaction(tx.hash)).to?.toLowerCase()).to.equal(
      to.toLowerCase(),
    );
  ...
  it('gets transaction count', async () => {
    expect(
      await provider.getTransactionCount(Wallet.createRandom().address),
    ).to.eq(0);
  });
